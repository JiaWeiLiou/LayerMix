...
void DrawAbsGraySystem(InputArray _field, OutputArray _grayField)
{
	...
	else if (field.type() == CV_16SC1)
	{
		// Find max value
		for (int i = 0; i < field.rows; ++i)
			for (int j = 0; j < field.cols; ++j)
				maxvalue = maxvalue > abs(field.at<short>(i, j)) ? maxvalue : abs(field.at<short>(i, j));

		//linear stretch to 255
		for (int i = 0; i < field.rows; ++i)
			for (int j = 0; j < field.cols; ++j)
				grayField.at<uchar>(i, j) = ((float)abs(field.at<short>(i, j)) / maxvalue) * 255;
	}
	else if (field.type() == CV_32FC1)
	{
		// Find max value
	...
	else if (field.type() == CV_16SC2)
	{
		// Find max value
		for (int i = 0; i < field.rows; ++i)
			for (int j = 0; j < field.cols; ++j)
			{
				short fx = field.at<Vec2s>(i, j)[0];
				short fy = field.at<Vec2s>(i, j)[1];
...
void CalculateGradient(InputArray _gradientField, OutputArray _gradm, OutputArray _gradd)
{
	...
	_gradm.create(gradientField.size(), CV_32FC1);
	...
	for (int i = 0; i < gradientField.rows; ++i)
		for (int j = 0; j < gradientField.cols; ++j)
		{
			...
			gradm.at<float>(i, j) = sqrt(x*x + y*y);
			gradd.at<float>(i, j) = atan2(-y, -x);
		}
}
