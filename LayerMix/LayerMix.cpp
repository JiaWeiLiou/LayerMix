...
int main()
{
	...
	BWConnectBreakLine(gradmSSCBL, graddSSCBL, lineSSCBL, gradmSACBL, graddSACBL, lineSACBL, 2, 10, 180, 1, 1);
	...
	BWConnectBreakLine(gradmSACBL, graddSACBL, lineSACBL, gradmLSCBL, graddLSCBL, lineLSCBL, 2, 100, 90, 0, 0);
	...
	string LLSCBL_outfile = filepath + "\\" + infilename + "_12.3.0_LSCBL.png";					//長對稱端點連通(二值)
	imwrite(LLSCBL_outfile, lineLSCBL);
	string gradmLSCBL_outfile = filepath + "\\" + infilename + "_12.3.1_LSCBLM.png";			//長對稱端點連通(幅值)
	imwrite(gradmLSCBL_outfile, gradmLSCBL_out);
	string graddLSCBL_outfile = filepath + "\\" + infilename + "_12.3.2_LSCBLD.png";			//長對稱端點連通(方向)
	imwrite(graddLSCBL_outfile, graddLSCBL_out);
	string gradfLSCBL_outfile = filepath + "\\" + infilename + "_12.3.3_LSCBLF.png";			//長對稱端點連通(場)
	imwrite(gradfLSCBL_outfile, gradfLSCBL_out);

	/*去除雜訊端點*/

	Mat lineCNP;	//去除雜訊端點(8UC1(BW))
	ClearSpecialPoint(lineLSCBL, lineCNP, blurLineSize, 2, 1);
	string LCNP_outfile = filepath + "\\" + infilename + "_12.4_CNP.png";			//去除雜訊端點
	imwrite(LCNP_outfile, lineCNP);

	/*長斷線強制連通*/
	
	Mat gradmLACBL, graddLACBL, lineLACBL;			//長斷線強制連通(8UC1、32FC1、8UC1(BW))
	BWConnectBreakLine(gradmLSCBL, graddLSCBL, lineCNP, gradmLACBL, graddLACBL, lineLACBL, 2, 20, 90, 1, 0);
	...
	string LLACBL_outfile = filepath + "\\" + infilename + "_12.5.0_LACBL.png";					//長斷線強制連通(二值)
	imwrite(LLACBL_outfile, lineLACBL);
	string gradmLACBL_outfile = filepath + "\\" + infilename + "_12.5.1_LACBLM.png";			//長斷線強制連通(幅值)
	imwrite(gradmLACBL_outfile, gradmLACBL_out);
	string graddLACBL_outfile = filepath + "\\" + infilename + "_12.5.2_LACBLD.png";			//長斷線強制連通(方向)
	imwrite(graddLACBL_outfile, graddLACBL_out);
	string gradfLACBL_outfile = filepath + "\\" + infilename + "_12.5.3_LACBLF.png";			//長斷線強制連通(場)
	imwrite(gradfLACBL_outfile, gradfLACBL_out);

	/*去除雜線*/

	Mat lineCNL;	//去除雜線(8UC1(BW))
	ClearSpecialPoint(lineLACBL, lineCNL, blurLineSize, 1000, 1);
	string LCNL_outfile = filepath + "\\" + infilename + "_12.6_CNL.png";			//去除雜線
